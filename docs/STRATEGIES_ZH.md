...
多智能体进攻模式。4 个子 Agent 以 `StructuredTool` 形式注册进 think-node `ToolRegistry`：
...
3 专家陪审团，以 `StructuredTool` 形式注册进 think-node `ToolRegistry`：**technicalAnalyst**、**trendAnalyst**、**riskAssessor**。
